...
export interface EditLeagueService {
  edit(id: string, updateLeagueDTO: UpdateLeagueDTO): Promise<League>;
}
...
export class LeagueService implements BackendLeagueService {
  ...
  async add(leagueDTO: CreateLeagueDTO) {
    ...
    const newLeague: League = new League(leagueDTO);
  ...
  async edit(id: string, updateLeagueDTO: UpdateLeagueDTO) {
